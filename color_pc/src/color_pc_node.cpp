...
public:
  ImageLivoxFusion():it(nh)
  {
    ...
    image_sub = it.subscribe("/camera/color/image_raw", 100, &ImageLivoxFusion::imageCallback, this);
...
void * ImageLivoxFusion::publish_thread(void * args)
{
  ...
  while (ros::ok())
  {
    ...
    if (is_rec_image && is_rec_lidar)
    {
      ...
      this_sub->colored_msg.header.frame_id = "livox";
...
void ImageLivoxFusion::set_param() 
{
  ...
  // 2. 关键修改：只取前 3 行！
// Rect(列起点, 行起点, 宽度, 高度) -> (0, 0, 4, 3)
// 这样 ext_3x4 就是一个 3x4 的矩阵
  cv::Mat ext_3x4 = ext_(cv::Rect(0, 0, 4, 3));

  // 3. 赋值给 extrinsic_matrix
  this->extrinsic_matrix = ext_3x4.clone();
